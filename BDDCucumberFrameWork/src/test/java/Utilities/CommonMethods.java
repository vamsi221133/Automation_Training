...
import StepDefinition.Mynthra_StpDef;


public class CommonMethods extends Mynthra_StpDef {
	public static WebElement norm_method(By by) {
		WebElement ele = BaseSetUpDriver_Mynthra.getDriver().findElement(by);
		return ele;
	}
	public static void clickMethod(By by)
	{
		WebElement we=BaseSetUpDriver_Mynthra.getDriver().findElement(by);
		we.click();
	}
	public static void sendKeysMethod(By byy,String args) throws IOException
	{
		Properties p=new Properties();
		BufferedReader rr=new BufferedReader(new FileReader("C:\\Users\\training\\Desktop\\BDDCucumberFrameWork\\PropertyList\\Mynthra.properties"));
		p.load(rr);
		WebElement we=BaseSetUpDriver_Mynthra.getDriver().findElement(byy);
